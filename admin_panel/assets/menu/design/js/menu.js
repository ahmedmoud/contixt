...
$.ajaxSetup({
	...
	$('body').on('click', '.edit-menu', function() {
		...
		gbox.show({
			type: 'ajax',
			url: base_url+'/admin/menu/editRow/'+menu_id,
			buttons: {
				'Save': function() {
		...
		gbox.show({
			...
			buttons: {
				'Yes': function() { 
					...
					$.post( base_url+'admin/menu/deleteArow/'+the_id , param, function(data) {
						if (data.success) {
							gbox.hide();
							li.remove();
						} else {
	...
	$('#edit-group').click(function() {
		...
		inputgroup.focus().select().keydown(function(e) {
			if (e.which == 13) {
				...
				$.ajax({
					type: 'POST',
					url: base_url+"admin/menu/editRow",
