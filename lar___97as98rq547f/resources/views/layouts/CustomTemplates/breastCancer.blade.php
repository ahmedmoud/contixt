...
<div class="col-md-12 nopadding">  <div class="beauty-box ___5"> <div class="title-header">  <h4><a class="nostylea" href="">{{ $output->thirdSection['title'] }}</a></h4></div>
@php 
$firstPost = $output->thirdSection['posts'][0];
@endphp
<div class="col-md-3 col-sm-6 col-xs-12">      
        <ul class="beauty-ul list-ul">
@foreach ( array_except( $output->thirdSection['posts'], [0,6,7,8,9,10]) as $k=>$post )

        <a href="{{ url( $post->slug ) }}">
            <li> <div class="list-img"> <img src="{{ Media::ClearifyAttach($post->Murl,'tiny') }}"> </div> <div class="list-info"> <h5>{{ $post->title }}</h5> </div> </li> </a>
        @endforeach
 </ul> </div>

         
<div class="col-md-6 col-sm-6 col-xs-12 nopadding"> <!-- Post -->  <div> <div class="b-box"> <a href="{{ url( $firstPost->slug ) }}"> <div class="b-img-cont"> <img src="{{ Media::ClearifyAttach($firstPost->Murl,'max') }}" class=" img-responsive d-block w-100">  </div> </a> <a href="{{ url($firstPost->slug) }}"><div class="slider-info"> <p class="h2 text-center">{{ $firstPost->title }}</p>  </div> </a> </div> </div> <!-- /Post -->  </div> 



<div class="col-md-3 col-sm-6 col-xs-12">      
<ul class="beauty-ul list-ul">
    @foreach ( array_except( $output->thirdSection['posts'], [0,1,2,3,4,5]) as $k=>$post )
    <a href="{{ url( $post->slug ) }}">
    <li> <div class="list-img"> <img src="{{ Media::ClearifyAttach($post->Murl,'tiny') }}"> </div> <div class="list-info"> <h5>{{ $post->title }}</h5> </div> </li> </a>
@endforeach
 </ul> </div>
...
@php $post = $output->fourthSection['posts'][0]; @endphp
     <div class="beauty-box   ___12"> <div class="title-header">  <h4><a class="nostylea" href=""> {{ $output->fourthSection['title'] }} </a></h4>  </div> <div class="col-md-6 col-sm-6 col-xs-12 nopadding"> <div class=""> <!-- Post -->  <div>  <div class="b-box">  <a href="{{ url( $post->slug ) }}">   <a href="{{ url( $post->slug ) }}">    <div class="b-img-cont"> <img src="{{ Media::ClearifyAttach($post->Murl,'max') }}" class=" img-responsive d-block w-100">    </div>  </a> <a href="{{ url( $post->slug ) }}"><div class="slider-info"> <p class="h2"> {{ $post->title }} </p> </div> </a>  </div> </div> <div class="love-life life-box onlyDesktop"> <div> 
            
            <ul class="list-ul">  

@foreach ( array_except($output->fourthSection['posts'], [0]) as $k=>$post )
@if( $k > 2 ) @break @endif
    <a href="{{ url( $post->slug ) }}">
    <li> <div class="list-img"> <img src="{{ Media::ClearifyAttach($post->Murl,'tiny') }}"> </div> <div class="list-info"> <h5>{{ $post->title }}</h5> </div> </li> </a>
@endforeach

</ul></div> </div> <!-- /Post -->   </div>  </div>  

@foreach ( array_only($output->fourthSection['posts'], [3,4,5,6]) as $post )
<div class="col-sm-3 col-xs-12 no-pad-left"> <div> <div class="b-box"><a href="{{ url( $post->slug ) }}"> <div class="b-img"> <img src="{{ Media::ClearifyAttach($post->Murl,'medium') }}" class=" img-responsive d-block w-100"> </div> </a> <a href="{{ url( $post->slug ) }}"> <div class="slider-info"> <p class="h4">{{ $post->title }}</p> </div> </a> </div> </div> </div>  
...
@endforeach
 ...
 <div class="col-md-4">
     <div class="love-life life-box onlyDesktop">
            <div class="title-header">  <h4><a class="nostylea" href="">{{ $output->fifthSection['title'] }}</a></h4>  </div>
        <ul class="list-ul"> 
@foreach ( array_only($output->fifthSection['posts'], [0,1]) as $post )
        <a href="#"> <li> <div class="list-ul-img" style=" height: 59px; "> <img src="{{ Media::ClearifyAttach($post->Murl,'tiny') }}"> </div> <h5> {{ $post->title }}</h5> </li> 
        </a>
    @endforeach
