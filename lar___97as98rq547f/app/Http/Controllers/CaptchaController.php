...
function simple_php_captcha($config = array()) {
    ...
    @session_start();
    $_SESSION['capCode'] = $captcha_config['code'];

    // Generate HTML for image src
    if ( strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) ) {
        $image_src = 'https://www.setaat.com/getCaptchaIMg?_CAPTCHA&amp;t=' . urlencode(microtime());
...
if( isset($_GET['_CAPTCHA']) ) {
    ...
    $sim = $this->simple_php_captcha();
   
    $captcha_config = unserialize($sim['config']);
 /*
