...
class MenuController extends Controller
{
        ...
        if (isset($this->data[$parent])) { 
            ...
            foreach ($this->data[$parent] as $key=>$row) {
               ...
               // if( $row['category_id'] ){
                ...
                $html .= '<li class="dropdown  tab-li _'.$key.' " '. $row['li_attr'] . '>';
                $html .= $row['label'];
    ...
    $compiled = \Cache::remember('easy_menu__'.$id.\APP::getLocale(), 60*60, function() use ($group_id, $attr, $way) {
...
if($row->category){ 
  ...
  //  $row->category = Category::select('slug','id')->where('id', $row->category)->first();
            ...
            $label = '<a href="'.url($row['slug']).'" '.$expand_data.'>';
            $label .= $row['title'];
            $label .= '</a>';
    
    
}else{
            $label = '<a href="'.$row['url'].'" '.$expand_data.'>';
            $label .= $row['title'];
            $label .= '</a>';
}
            ...
            $li_attr = '';
            if($row->category){
            	$row['class'] = 'tab-li drop _'.$kk.' ';
            	
            	$category_id = $row['category'];
            	
            	
            }else { $category_id = false;  $row['class'] = 'tab-li drop _'.$kk.' '; }
